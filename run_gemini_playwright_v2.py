...
def heuristic_extract_blocks(text):
    ...
    mappings = {
        ...
        'VISUAL-SPEC': [r'VISUAL-SPEC', r'Visual Specification', r'Technical Spec'],
        'RENDERED-CODE': [r'RENDERED-CODE', r'Code Block', r'Executable Code', r'CODE'],
        'USAGE-GUIDE': [r'USAGE-GUIDE', r'Usage Guide', r'Usage block', r'Interaction Guide'],
    }
    ...
    for chunk in chunks:
        ...
        if not found_header:
            if re.search(r'\{\s*"training_data_id"', chunk):
                current_block = 'METADATA'
                blocks['METADATA'] = (blocks.get('METADATA', "") + "\n" + chunk).strip()
            elif current_block:
...
def validate_and_save_json(llm_response, out_json_path, thinking_text=None):
    ...
    try:
        ...
        # ── Preamble Think-Block Rescue ────────────────────────────────────────
        # Gemini sometimes places <think>...</think> BEFORE !!!!!METADATA!!!!!
        # as a JSON reasoning preamble. If our REASONING block is thin, rescue it.
        if len(reasoning_main) < 500:
            preamble_match = re.search(r'<think>(.*?)</think>', llm_response, re.DOTALL | re.IGNORECASE)
            if preamble_match:
                rescued = preamble_match.group(1).strip()
                if len(rescued) > len(reasoning_main):
                    log(f"  [ThinkRescue] REASONING block thin ({len(reasoning_main)} chars). "
                        f"Rescued preamble think block ({len(rescued)} chars).")
                    reasoning_main = rescued

        # 3. Extraction: Assistant Content
        visual_spec = clean_semantic_block(blocks.get("VISUAL-SPEC", ""))
        rendered_code = clean_semantic_block(blocks.get("RENDERED-CODE", ""))
        usage_guide = clean_semantic_block(blocks.get("USAGE-GUIDE", ""))

        # Handle fragmented code parts just in case
        all_code_parts = []
        code_keys = sorted([k for k in blocks.keys() if k.startswith("RENDERED-CODE-PART-")], 
        ...
        # Merge results into final object specific to Visual Tasks
        assistant_content_obj = {
            "technical_visual_specification": visual_spec,
            "rendered_code": rendered_code,
            "usage_and_interaction_guide": usage_guide
        }
        ...
        # Infer task_type from prompt filename if missing from metadata
        if "task_type" not in final_task or not final_task.get("task_type"):
            # Try to recover from the output file path  
            fname = os.path.basename(out_json_path)
            for tt in ["html_tool", "html_presentation", "plantuml_diagram", "graphviz_dot",
                       "d2_diagram", "tikz_pgfplots", "mermaid_diagram", "svg_generation"]:
                if tt.replace("_", "") in rendered_code.replace(" ", "").lower():
                    final_task["task_type"] = tt
                    log(f"  [TypeInfer] Inferred task_type='{tt}' from rendered_code signature")
                    break
            if "task_type" not in final_task:
                final_task["task_type"] = "visual_task"
    ...
    with sync_playwright() as p:
        ...
        try:
            page.evaluate("""() => {
                ...
                // Check if sidebar is expanded via side-navigation-v2 component width
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    model_info = page.evaluate("""() => {
                        const options = document.querySelectorAll('button.bard-mode-list-button, gem-menu-item[role="menuitem"]');
                        if (options.length === 0) {
                        ...
                        allButtons.forEach((btn, idx) => {
                            ...
                            const isSelected = btn.classList.contains('is-selected') || btn.classList.contains('selected');
