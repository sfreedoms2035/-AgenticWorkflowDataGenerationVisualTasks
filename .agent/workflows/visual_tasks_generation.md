---
description: Automatically orchestrates the generation of 16 visual/architectural tasks per PDF using Playwright and Gemini Web App.
---

# VisualTasksGenerationWorkflow

This workflow generates 16 expert-level AD/ADAS visual tasks per PDF by automating the Gemini web interface via `pipeline.py`.
...
python pipeline.py
```
Optionally with render preview:
```bash
python pipeline.py --render-preview
```
...
```
**How to determine resume point:** Read `progress.json`, find the last task with `"status": "PASS"`, and start from the next task in the sequence.
...
2. **Builds** a full prompt per turn/task/variation (task type, role, strategy, complete 8-step CoT template)
3. **Runs** `run_gemini_playwright_v2.py` (always selects Gemini Pro, zero manual steps)
4. **Validates** output via `validate_task.py` (visual-adapted quality gates)
...
9. **(Optional)** Renders preview in browser when `--render-preview` is passed
...
3. If local fix: `python .agent/scripts/auto_repair.py Output\json\{Doc}_Turn{N}_Task{K}.json`
4. Re-validate: `python .agent/scripts/validate_task.py Output\json\{Doc}_Turn{N}_Task{K}.json`
5. If needs regeneration: `python pipeline.py --pdf "file.pdf" --turn N --task K`
...
| Visual code volume | Dynamic by task type (100-600 lines) |
...
- **Gemini re-prompt**: volume failures (CoT/answer too short), missing CoT elements, immersion breaks
...
- **Always resume, never restart.** Use `--resume` or explicit `--turn/--task` flags.
- **Optimize freely.** The agent can modify workflow files, scripts, and skills to improve quality, speed, or reliability.
- **Log what you change.** If you modify a script or skill, note it in the conversation.
